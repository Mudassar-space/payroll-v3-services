...
from app.cores.managers.master_data.country import CountryManager
...
country_router = InferringRouter()
...
class CountryController():
    ...
    @country_router.post("/",
                          ...
                          description="Create a Country and return Country with details",
                          status_code=status.HTTP_201_CREATED,
                          response_model=CountryRequestPayload,
    ...
    async def create(self, payload: CountryRequestPayload, token_data=Depends(JWTAuthentication())):
        try:
            response = await self.country_manager.create(payload)
    ...
    @country_router.get("/{id}/",
                ...
                description="Returns Country detail",
                responses={status.HTTP_200_OK: {"model": CountryResponsePayload},
                           status.HTTP_401_UNAUTHORIZED: {"model": ResponseMessage},
    ...
    async def get_by_id(self, id: str, token_data=Depends(JWTAuthentication())):
        try:
            response = await self.country_manager.get_by_id(id)
            if not isinstance(response, ResponseMessage):
    ...
    @country_router.delete("/{id}/",
                   ...
                   description="Returns confirmation if Country is deleted or not",
    ...
    async def delete_by_id(self, id: str, token_data=Depends(JWTAuthentication())):
        try:
            response = await self.country_manager.delete_by_id(id)
            if response.success:
                return JSONResponse(status_code=status.HTTP_200_OK, content=jsonable_encoder(response))
            return JSONResponse(status_code=status.HTTP_404_NOT_FOUND, content=jsonable_encoder(response))
        except Exception as e:
    ...
    @country_router.put("/{id}/",
                ...
                description="Update the Country by id and return the updated record",
                responses={status.HTTP_200_OK: {"model": CountryResponsePayload},
                           ...
                           status.HTTP_422_UNPROCESSABLE_ENTITY: {"model": ResponseMessage},
                           status.HTTP_500_INTERNAL_SERVER_ERROR: {"model": ResponseMessage}})
    async def update_by_id(self, id: str, payload: CountryRequestPayload, token_data=Depends(JWTAuthentication())):
        try:
            response = await self.country_manager.update_by_id(id, payload)
            if not isinstance(response, ResponseMessage):
                return JSONResponse(status_code=status.HTTP_200_OK, content=jsonable_encoder(response))
            return JSONResponse(status_code=status.HTTP_400_BAD_REQUEST, content=jsonable_encoder(response))
        except Exception as e:
    ...
    @country_router.get("/",
                ...
                description="Returns a List containing Country Objects",
                responses={status.HTTP_200_OK: {"model": CountryListResponse},
                           status.HTTP_401_UNAUTHORIZED: {"model": ResponseMessage},
                           status.HTTP_422_UNPROCESSABLE_ENTITY: {"model": ResponseMessage},
                           status.HTTP_500_INTERNAL_SERVER_ERROR: {"model": ResponseMessage}})
    async def get_list(self, search_query: str = "", order_by: str = "name",
        ...
        try:
            ...
            response = await self.country_manager.get_list(params)
            return JSONResponse(status_code=status.HTTP_200_OK, content=jsonable_encoder(response))
        except Exception as e:
            self.logger.error(e)
            message = ResponseMessage(
    ...
    @country_router.get("/ddl",
                response_model=List[CountryDdlListResponse],
    ...
    async def get_dll_list(self, token_data=Depends(JWTAuthentication())):
        try:
            response = await self.country_manager.get_ddl_list()
            return JSONResponse(status_code=status.HTTP_200_OK, content=jsonable_encoder(response))
        except Exception as e:
            self.logger.error(e)
            print(e)
