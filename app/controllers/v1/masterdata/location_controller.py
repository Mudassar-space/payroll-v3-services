...
from app.cores.managers.master_data.location_manager import LocationManager
...
from app.models.master_data.location import LocationListResponseModel, LocationRequestPayload, LocationResponsePayload
...
class LocationController():
    ...
    @location_router.post("/",
                          ...
                          description="Create a Location and return Location with details",
                          status_code=status.HTTP_201_CREATED,
                          response_model=LocationResponsePayload,
    ...
    async def create(self,client_id:str, payload: LocationRequestPayload, token_data=Depends(JWTAuthentication())):
        try:
            ...
            location_payload = LocationResponsePayload(**dict(payload),
                                        client_id=client_id,
                                        created_by_external_id=created_by_external_id,
                                        created_by_email=created_by_email,
                                        )
            response = await self.location_manager.create(location_payload)
    ...
    @location_router.get("/{id}/",
                ...
                description="Returns Location detail",
                responses={status.HTTP_200_OK: {"model": LocationResponsePayload},
                           status.HTTP_401_UNAUTHORIZED: {"model": ResponseMessage},
    ...
    async def get_by_id(self, id: str, token_data=Depends(JWTAuthentication())):
        try:
            response = await self.Location_manager.get_by_id( id)
            if not isinstance(response, ResponseMessage):
    ...
    @location_router.delete("/{id}/",
                   ...
                   description="Returns confirmation if Location is deleted or not",
    ...
    async def delete_by_id(self, id: str, token_data=Depends(JWTAuthentication())):
        try:
            response = await self.Location_manager.delete_by_id(id)
            if response.success:
    ...
    @location_router.put("/{id}/",
                ...
                description="Update the Location by id and return the updated record",
                responses={status.HTTP_200_OK: {"model": LocationResponsePayload},
                           ...
                           status.HTTP_422_UNPROCESSABLE_ENTITY: {"model": ResponseMessage},
                           status.HTTP_500_INTERNAL_SERVER_ERROR: {"model": ResponseMessage}})
    async def update_by_id(self, client_id:str, id: str, payload: LocationRequestPayload, token_data=Depends(JWTAuthentication())):
        try:
            partner_token_data_dict = dict(token_data)
            ...
            response = await self.location_manager.update_by_id(id, location_payload)
            if not isinstance(response, ResponseMessage):
                return JSONResponse(status_code=status.HTTP_200_OK, content=jsonable_encoder(response))
            return JSONResponse(status_code=status.HTTP_400_BAD_REQUEST, content=jsonable_encoder(response))
        except Exception as e:
    ...
    @location_router.get("/",
                ...
                description="Returns a List containing Location Objects",
                responses={status.HTTP_200_OK: {"model": LocationListResponseModel},
                           status.HTTP_401_UNAUTHORIZED: {"model": ResponseMessage},
                           status.HTTP_422_UNPROCESSABLE_ENTITY: {"model": ResponseMessage},
                           status.HTTP_500_INTERNAL_SERVER_ERROR: {"model": ResponseMessage}})
    async def get_list(self, client_id:str, search_query: str = "", order_by: str = "name",
        ...
        try:
            ...
            response = await self.location_manager.get_list(client_id, params)
            return JSONResponse(status_code=status.HTTP_200_OK, content=jsonable_encoder(response))
        except Exception as e:
            self.logger.error(e)
            print(e)
    ...
    @location_router.get("/location-ddl",
                response_model=List[DDLEntity],
                summary="Get Location ddl List",
                description="Returns a dll List containing Location Objects",
                responses={
                           status.HTTP_401_UNAUTHORIZED: {"model": ResponseMessage},
                           status.HTTP_422_UNPROCESSABLE_ENTITY: {"model": ResponseMessage},
                           status.HTTP_500_INTERNAL_SERVER_ERROR: {"model": ResponseMessage}})
    async def get_dll_list(self,client_id:str,  token_data=Depends(JWTAuthentication())):
        try:
            response = await self.location_manager.get_ddl_list(client_id)
            return JSONResponse(status_code=status.HTTP_200_OK, content=jsonable_encoder(response))
        except Exception as e:
            self.logger.error(e)
            print(e)
