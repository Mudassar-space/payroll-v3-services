...
from app.cores.managers.master_data.deduction_manager import DeductionManager
...
from app.models.master_data.deduction import DeductionListResponseModel, DeductionRequestPayload, DeductionResponsePayload

deduction_router = InferringRouter()


@cbv(deduction_router)
class DeductionController():
    def __init__(self):
        self.deduction_manager = DeductionManager()
        self.logger = Logger()

    @deduction_router.post("/",
                          summary="Create a Deduction",
                          description="Create a Deduction and return Deduction with details",
                          status_code=status.HTTP_201_CREATED,
                          response_model=DeductionResponsePayload,
    ...
    async def create(self, payload: DeductionRequestPayload, token_data=Depends(JWTAuthentication())):
        try:
            ...
            deduction_payload = DeductionResponsePayload(**dict(payload),
                                        created_by_external_id=created_by_external_id,
                                        created_by_email=created_by_email,
                                        )
            response = await self.deduction_manager.create(deduction_payload)
    ...
    @deduction_router.get("/{id}/",
                summary="Get Deduction by id",
                description="Returns Deduction detail",
                responses={status.HTTP_200_OK: {"model": DeductionResponsePayload},
                           status.HTTP_401_UNAUTHORIZED: {"model": ResponseMessage},
    ...
    async def get_by_id(self, id: str, token_data=Depends(JWTAuthentication())):
        try:
            response = await self.deduction_manager.get_by_id(id)
            if not isinstance(response, ResponseMessage):
    ...
    @deduction_router.delete("/{id}/",
                   summary="Delete Deduction by id",
                   description="Returns confirmation if Deduction is deleted or not",
    ...
    async def delete_by_id(self, id: str, token_data=Depends(JWTAuthentication())):
        try:
            response = await self.deduction_manager.delete_by_id(id)
            if response.success:
    ...
    @deduction_router.put("/{id}/",
                summary="Update Deduction by id",
                description="Update the Deduction by id and return the updated record",
                responses={status.HTTP_200_OK: {"model": DeductionResponsePayload},
                           ...
                           status.HTTP_422_UNPROCESSABLE_ENTITY: {"model": ResponseMessage},
                           status.HTTP_500_INTERNAL_SERVER_ERROR: {"model": ResponseMessage}})
    async def update_by_id(self, id: str, payload: DeductionRequestPayload, token_data=Depends(JWTAuthentication())):
        try:
            partner_token_data_dict = dict(token_data)
            ...
            response = await self.deduction_manager.update_by_id(id, deduction_payload)
            if not isinstance(response, ResponseMessage):
                return JSONResponse(status_code=status.HTTP_200_OK, content=jsonable_encoder(response))
            return JSONResponse(status_code=status.HTTP_400_BAD_REQUEST, content=jsonable_encoder(response))
        except Exception as e:
    ...
    @deduction_router.get("/",
                summary="Get Deduction List",
                description="Returns a List containing Deduction Objects",
                responses={status.HTTP_200_OK: {"model": DeductionListResponseModel},
                           status.HTTP_401_UNAUTHORIZED: {"model": ResponseMessage},
                           status.HTTP_422_UNPROCESSABLE_ENTITY: {"model": ResponseMessage},
                           status.HTTP_500_INTERNAL_SERVER_ERROR: {"model": ResponseMessage}})
    async def get_list(self, client_id:str, search_query: str = "", order_by: str = "name",
        ...
        try:
            ...
            response = await self.deduction_manager.get_list(client_id, params)
            return JSONResponse(status_code=status.HTTP_200_OK, content=jsonable_encoder(response))
        except Exception as e:
            self.logger.error(e)
            print(e)
    ...
    @deduction_router.get("/deduction-ddl",
                response_model=List[DDLEntity],
                summary="Get Deduction ddl List",
                description="Returns a dll List containing Deduction Objects",
                responses={
                           status.HTTP_401_UNAUTHORIZED: {"model": ResponseMessage},
                           status.HTTP_422_UNPROCESSABLE_ENTITY: {"model": ResponseMessage},
                           status.HTTP_500_INTERNAL_SERVER_ERROR: {"model": ResponseMessage}})
    async def get_dll_list(self,client_id:str,  token_data=Depends(JWTAuthentication())):
        try:
            response = await self.deduction_manager.get_ddl_list(client_id)
            return JSONResponse(status_code=status.HTTP_200_OK, content=jsonable_encoder(response))
        except Exception as e:
            self.logger.error(e)
            print(e)
