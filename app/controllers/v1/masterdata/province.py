...
from app.cores.managers.master_data.province import ProvinceManager
...
class ProvinceController():
    ...
    @province_router.post("/",
                          ...
                          description="Create a Province and return Province with details",
                          status_code=status.HTTP_201_CREATED,
                          response_model=ProvinceRequestPayload,
    ...
    async def create(self, payload: ProvinceRequestPayload, token_data=Depends(JWTAuthentication())):
        try:
            response = await self.province_manager.create(payload)
    ...
    @province_router.get("/{id}/",
                summary="Get Province by id",
                description="Returns Province detail",
                responses={status.HTTP_200_OK: {"model": ProvinceResponsePayload},
                           status.HTTP_401_UNAUTHORIZED: {"model": ResponseMessage},
    ...
    async def get_by_id(self, id: str, token_data=Depends(JWTAuthentication())):
        try:
            response = await self.province_manager.get_by_id(id)
            if not isinstance(response, ResponseMessage):
    ...
    @province_router.delete("/{id}/",
                   summary="Delete Province by id",
                   description="Returns confirmation if Province is deleted or not",
    ...
    async def delete_by_id(self, id: str, token_data=Depends(JWTAuthentication())):
        try:
            response = await self.province_manager.delete_by_id(id)
            if response.success:
    ...
    @province_router.put("/{id}/",
                summary="Update Province by id",
                description="Update the Province by id and return the updated record",
                responses={status.HTTP_200_OK: {"model": ProvinceResponsePayload},
                           ...
                           status.HTTP_422_UNPROCESSABLE_ENTITY: {"model": ResponseMessage},
                           status.HTTP_500_INTERNAL_SERVER_ERROR: {"model": ResponseMessage}})
    async def update_by_id(self, id: str, payload: ProvinceRequestPayload, token_data=Depends(JWTAuthentication())):
        try:
            response = await self.province_manager.update_by_id(id, payload)
            if not isinstance(response, ResponseMessage):
                return JSONResponse(status_code=status.HTTP_200_OK, content=jsonable_encoder(response))
            return JSONResponse(status_code=status.HTTP_400_BAD_REQUEST, content=jsonable_encoder(response))
        except Exception as e:
    ...
    @province_router.get("/",
                ...
                description="Returns a List containing Province Objects",
                responses={status.HTTP_200_OK: {"model": ProvinceListResponse},
                           status.HTTP_401_UNAUTHORIZED: {"model": ResponseMessage},
                           status.HTTP_422_UNPROCESSABLE_ENTITY: {"model": ResponseMessage},
                           status.HTTP_500_INTERNAL_SERVER_ERROR: {"model": ResponseMessage}})
    async def get_list(self, search_query: str = "", order_by: str = "name",
        ...
        try:
            ...
            response = await self.province_manager.get_list(params)
            return JSONResponse(status_code=status.HTTP_200_OK, content=jsonable_encoder(response))
        except Exception as e:
            self.logger.error(e)
            message = ResponseMessage(
    ...
    @province_router.get("/ddl",
                response_model=List[ProvinceDdlListResponse],
    ...
    async def get_dll_list(self, country_id: str, token_data=Depends(JWTAuthentication())):
        try:
            response = await self.province_manager.get_ddl_list(country_id)
            return JSONResponse(status_code=status.HTTP_200_OK, content=jsonable_encoder(response))
        except Exception as e:
            self.logger.error(e)
            print(e)
