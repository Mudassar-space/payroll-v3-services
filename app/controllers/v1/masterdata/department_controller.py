...
from app.cores.managers.master_data.department_manager import DepartmentManager
...
department_router = InferringRouter()
...
class DepartmentController():
    ...
    @department_router.post("/",
                          ...
                          description="Create a Department and return Department with details",
                          status_code=status.HTTP_201_CREATED,
                          response_model=DepartmentRequestPayload,
    ...
    async def create(self,client_id:str, payload: DepartmentRequestPayload, token_data=Depends(JWTAuthentication())):
        try:
            ...
            department_payload = DepartmentResponsePayload(**dict(payload),
                                        client_id=client_id,
                                        created_by_external_id=created_by_external_id,
                                        created_by_email=created_by_email,
                                        )
            response = await self.department_manager.create(department_payload)
    ...
    @department_router.get("/{id}/",
                ...
                description="Returns Department detail",
                responses={status.HTTP_200_OK: {"model": DepartmentResponsePayload},
                           status.HTTP_401_UNAUTHORIZED: {"model": ResponseMessage},
    ...
    async def get_by_id(self, id: str, token_data=Depends(JWTAuthentication())):
        try:
            response = await self.department_manager.get_by_id(id)
            if not isinstance(response, ResponseMessage):
    ...
    @department_router.delete("/{id}/",
                   ...
                   description="Returns confirmation if Department is deleted or not",
    ...
    async def delete_by_id(self, id: str, token_data=Depends(JWTAuthentication())):
        try:
            response = await self.department_manager.delete_by_id(id)
            if response.success:
    ...
    @department_router.put("/{id}/",
                ...
                description="Update the Department by id and return the updated record",
                responses={status.HTTP_200_OK: {"model": DepartmentResponsePayload},
                           ...
                           status.HTTP_422_UNPROCESSABLE_ENTITY: {"model": ResponseMessage},
                           status.HTTP_500_INTERNAL_SERVER_ERROR: {"model": ResponseMessage}})
    async def update_by_id(self, id: str, payload: DepartmentRequestPayload, token_data=Depends(JWTAuthentication())):
        try:
            partner_token_data_dict = dict(token_data)
            ...
            response = await self.department_manager.update_by_id(id, department_payload)
            if not isinstance(response, ResponseMessage):
                return JSONResponse(status_code=status.HTTP_200_OK, content=jsonable_encoder(response))
            return JSONResponse(status_code=status.HTTP_400_BAD_REQUEST, content=jsonable_encoder(response))
        except Exception as e:
    ...
    @department_router.get("/",
                ...
                description="Returns a List containing Department Objects",
                responses={status.HTTP_200_OK: {"model": DepartmentListResponseModel},
                           status.HTTP_401_UNAUTHORIZED: {"model": ResponseMessage},
                           status.HTTP_422_UNPROCESSABLE_ENTITY: {"model": ResponseMessage},
                           status.HTTP_500_INTERNAL_SERVER_ERROR: {"model": ResponseMessage}})
    async def get_list(self,client_id:str, search_query: str = "", order_by: str = "name",
        ...
        try:
            ...
            response = await self.department_manager.get_list(client_id, params)
            return JSONResponse(status_code=status.HTTP_200_OK, content=jsonable_encoder(response))
        except Exception as e:
            self.logger.error(e)
            print(e)
    ...
    @department_router.get("/department-ddl",
                response_model=List[DDLEntity],
                summary="Get Departments ddl List",
                description="Returns a dll List containing Departments Objects",
                responses={
                           status.HTTP_401_UNAUTHORIZED: {"model": ResponseMessage},
                           status.HTTP_422_UNPROCESSABLE_ENTITY: {"model": ResponseMessage},
                           status.HTTP_500_INTERNAL_SERVER_ERROR: {"model": ResponseMessage}})
    async def get_dll_list(self,client_id:str, token_data=Depends(JWTAuthentication())):
        try:
            response = await self.department_manager.get_ddl_list(client_id)
            return JSONResponse(status_code=status.HTTP_200_OK, content=jsonable_encoder(response))
        except Exception as e:
            self.logger.error(e)
            print(e)
