...
class TaxYearRequestPayload(CamelModel):
    tax_year: int


class TaxYearResponsePayload(TaxYearRequestPayload,WhoCreateDataModel,WhoUpdateDataModel):
...
class TaxYearListModel(CamelModel):
    tax_year: int
    id: Optional[PyObjectId] = Field(default_factory=PyObjectId)
...
class TaxYearListResponseModel(CamelModel):
    page: Optional[int]
    size: Optional[int]
    total: Optional[int]
    items: Optional[List[TaxYearListModel]]
