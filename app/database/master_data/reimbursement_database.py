...
from app.models.master_data.reimbursement import ReimbursementResponsePayload


class ReimbursementDatabase():
    client = AsyncIOMotorClient(MONGODB_CON_STR)
    client.get_io_loop = asyncio.get_running_loop
    reimbursement_collection = client[DATABASE_NAME]["reimbursement"]
    ...
    async def create(self, payload: ReimbursementResponsePayload):
        ...
        result = await self.reimbursement_collection.insert_one(document)
        document["id"] = str(result.inserted_id)
        del document["_id"]
        return document
    
    def get_by_whereclause(self, where_clause):
        return self.reimbursement_collection.find_one(where_clause)
    
    async def delete_by_id(self, id: ObjectId):
        return await self.reimbursement_collection.delete_one({"_id": id})

    async def update_by_id(self, id: ObjectId, payload: ReimbursementResponsePayload):
        ...
        result = await self.reimbursement_collection.update_one({"_id": id}, {"$set": update_request_payload})
        return result
    ...
    async def get_all(self, client_id: str, params):
        ...
        cursor = self.reimbursement_collection.find(filter, {"label": 1, "showInPayroll": 1}).sort(
        ...
        total_count =await self.reimbursement_collection.aggregate([{
    ...
    async def get_ddl(self, client_id:str):
        reimbursement_ddl =await self.reimbursement_collection.aggregate([
            {
                "$match":{
