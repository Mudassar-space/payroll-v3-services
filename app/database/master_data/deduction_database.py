...
class DeductionDatabase():
    client = AsyncIOMotorClient(MONGODB_CON_STR)
    client.get_io_loop = asyncio.get_running_loop
    deduction_collection = client[DATABASE_NAME]["deduction"]
    ...
    async def create(self, payload: DeductionResponsePayload):
        ...
        result = await self.deduction_collection.insert_one(document)
        document["id"] = str(result.inserted_id)
        del document["_id"]
        return document
    
    def get_by_whereclause(self, where_clause):
        return self.deduction_collection.find_one(where_clause)
    
    async def delete_by_id(self, id: ObjectId):
        return await self.deduction_collection.delete_one({"_id": id})

    async def update_by_id(self, id: ObjectId, payload: DeductionResponsePayload):
        ...
        result = await self.deduction_collection.update_one({"_id": id}, {"$set": update_request_payload})
        return result
    ...
    async def get_all(self, client_id: str, params):
        ...
        cursor = self.deduction_collection.find(filter, {"label": 1, "showInPayroll": 1}).sort(
        ...
        total_count =await self.deduction_collection.aggregate([{
    ...
    async def get_ddl(self, client_id:str):
        deduction_ddl =await self.deduction_collection.aggregate([
            {
                "$match":{
                    "clientId":client_id,
                    },
